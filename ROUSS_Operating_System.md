...
El ejemplo más claro de una automatización que responde bien las cinco preguntas es la confirmación automática de intención por WhatsApp que ya opera en Dropper: elimina la necesidad de que un humano esté disponible a cualquier hora para confirmar un pedido nuevo; evita el error de despachar un pedido que nunca se iba a concretar; captura si el cliente respondió sí, no, o de forma ambigua; mejora la experiencia del comprador (confirmación casi inmediata en vez de esperar horas); y afecta directamente la tasa de flete evitado y la efectividad de entrega (Sección 10). Cualquier automatización nueva se evalúa contra este mismo estándar, no contra "qué tan sofisticada es técnicamente".

**Principios estables que se derivan del diseño real de Dropper, generalizados más allá del código:**
...
- **Dropper, tal como está diseñado hoy, se limita deliberadamente a capturar, confirmar y clasificar** — no a sostener conversaciones abiertas y complejas ni a reemplazar el juicio de un vendedor humano ante una objeción ambigua. Rocío no debe asumir que la automatización actual puede hacer más de lo que este alcance permite; para lo que excede ese alcance, la conversación sigue necesitando criterio humano o una capa de guion específica.

El detalle técnico de cómo está construido esto hoy (arquitectura, estado de despliegue, variables de configuración) vive en el mapa técnico propio de Dropper, que se actualiza por separado — este documento solo necesita los principios de arriba para razonar decisiones.
...
- **El proveedor logístico no avisa activamente cuando cambia el estado de un pedido — hay que preguntarle.** Esto significa que la frescura del estado "entregado" (el que dispara la postventa, Sección 9) depende de que alguien o algo consulte con regularidad; no es información que llegue sola.
...
Estas reglas provienen de la versión vigente de la documentación de Rocketfy al momento de escribir esto — si Rocketfy cambia su API o sus políticas, esta sección debe revisarse contra la documentación actualizada antes de asumir que sigue siendo exacta.
